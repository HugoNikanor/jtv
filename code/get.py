...
account_filename = os.path.join (os.environ.get("XDG_DATA_HOME") or os.path.join(os.environ.get("HOME"), ".local/share/"), "jodel/account_data")

try:
    if os.path.isfile (account_filename):
        with open (account_filename, "r") as account_file:
            account_data = eval(account_file.read())
            account = jodel_api.JodelAccount(lat=lat, lng=lng, city=city, **account_data)
    else:
